...
<div class="total">
  ...
  if(isset($_SESSION['accept'])){
    ...
    if($tem==true) {   
      ...
      <strong>Success </strong>Complaint id $cmp solved
      </div>";
  ...
  $fn=1;
  include('connect.php');     
   $query="SELECT * FROM `accept_complaints`";
   $res=mysqli_query($conn,$query);
   $norows=mysqli_num_rows($res);
   if($norows==0){
    echo "<div class='norows'>There are no Accepted Complaints found recently</div>";
  }
   ...
   <table>
    ...
    <d id='bt'>
      ...
      <form action='adminacceptcomp.php' method='POST'>
      <button class='btn'  name='accept' value=$fn>Complaint Solved</button>
  ...
  while($row=mysqli_fetch_assoc($result)){
      if($rvalue==$val){
        $id=$row['Complaintid'];
        $query="SELECT * FROM `accept_complaints` WHERE `Complaintid`=$id";
        $res=mysqli_query($conn,$query);
        ...
        $date=$rowr['Date'];
        $rdate=date('d/m/y');
        $query="INSERT INTO `solved_complaints` values($no,$cid,'$sub','$com','$date','$rdate')";
        mysqli_query($conn,$query); 
        $query="DELETE FROM `accept_complaints` WHERE `complaintid`=$id";
        mysqli_query($conn,$query);
        session_start();
        ...
        header("Location: adminacceptcomp.php");
      }
      $rvalue=$rvalue+1;
  }
     }
    ...
    while($row=mysqli_fetch_assoc($result)){
     if($rvalue==$val){
       ...
       $res=mysqli_query($conn,$query);      
       $qu="DELETE FROM `accept_complaints` WHERE `Complaintid`=$id";
