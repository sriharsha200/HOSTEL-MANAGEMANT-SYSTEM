...
<body>
    <h1>SENIOR BOYS HOSTEL ROOM STATUS</h1>
	<div class="container">
		<table>
            <tr>
              <th>First Floor</th>
             ...
             $room=101;$no_rooms=22;$count=1;
             $p1="dot";$p2="dot1";
             while($count<=$no_rooms){
              echo "<tr>";
             for($i=0;$i<4;$i++){
               $rows=0;
              $query="SELECT * FROM `second_years` WHERE `room`=($room+$i*100) AND `gender`='male'";  
              $result=mysqli_query($conn,$query);  
              $rows+=mysqli_num_rows($result); 
              $query="SELECT * FROM `third_years` WHERE `room`=($room+$i*100) AND `gender`='male'";  
              $result=mysqli_query($conn,$query);  
              $rows+=mysqli_num_rows($result); 
              $query="SELECT * FROM `final_years` WHERE `room`=($room+$i*100) AND `gender`='male'";  
              $result=mysqli_query($conn,$query);  
              $rows+=mysqli_num_rows($result); 
              if($rows==0){
  ...
  <br>
  <br>
  <form action="seniorboyshostel.php" method="post">
