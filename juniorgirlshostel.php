...
<body>
    <h1>JUNIOR GIRLS HOSTEL ROOM STATUS</h1>
	<div class="container">
		<table>
            <tr>
              <th>Ground Floor</th>
           ...
           <?php 
             ...
             $room=101;$no_rooms=6;$count=1;
             $p1="dot";$p2="dot1";
             while($count<=$no_rooms){
              echo "<tr>";
             for($i=0;$i<4;$i++){
              $query="SELECT * FROM `reg_details` WHERE `room`=($room+$i*100) AND `gender`='female'";  
             ...
             $s1='<td>'.($room+$i*100).'<div class="dots"><span class='.$p1.'></span><span class='.$p2.'></span></div></td>';
             echo $s1;
             }
             else{
               if($rows==0){
  ...
  <br>
  <br>
  <form action="juniorgirlshostel.php" method="post">
